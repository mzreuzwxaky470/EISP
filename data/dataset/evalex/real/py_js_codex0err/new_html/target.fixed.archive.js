...
///// Segment BEGIN htmlparser4 DONE
    ...
    parse_html_declaration(i) {
        ...
        if (rawdata.slice(i, i+4) === '<!--') {
            ...
            return this.parse_bogus_(i);
        }
    }

    // Internal -- parse bogus , return length or -1 if not terminated
    // see http://www.w3.org/TR/html5/tokenization.html#bogus--state
    parse_bogus_(i, report=1) {
        let rawdata = this.rawdata;
        console.assert(rawdata.slice(i, i+2) in ['<!', '</'], 'unexpected call to parse_()');
        let pos = rawdata.indexOf('>', i+2);
        if (pos === -1) {
            return -1;
        }
        if (report) {
            this.handle_(rawdata.slice(i+2, pos));
        }
        return pos + 1;
    }
