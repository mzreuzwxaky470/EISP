...
import sys
# from nvi import chat_llama3,chat_llama3_continue
model = None
tokenizer =None
pipeline_=None
# 
map_threshold = 3
prompt_path="/home/user/cl/EISP/prompt"
input_tokens = 0
output_tokens = 0
dependencies=[]
js_map_code_saved=""
# pipeline
# 
def map_code(source_code,target_code,model_=None,tokenizer_=None,pipeline__=None):
    global model, tokenizer,pipeline_
    model=model_
    tokenizer=tokenizer_
    pipeline_=pipeline__
    # return ""
    map_prompt=""
    ...
    # return chat_llama3(map_prompt)
    # response, messages, extra_response_count=Mistral_7B_v2_prompt(map_prompt, model, tokenizer)
    # res, messages, extra_response_count=get_api_response(map_prompt)
    # coding_prompt = f"Code User: {map_prompt}<|end_of_turn|>Code Assistant:"

    # single_turn_prompt = f"GPT4 Correct User: {map_prompt}<|end_of_turn|>GPT4 Correct Assistant:"
    # res=generate_response_starling_LM_7B_beta(single_turn_prompt, model, tokenizer)
    # follow_up_question="Note that all python and JavaScript statements need to be tagged with \"# --- py stmt\" at the end."
    # multi_turn_prompt = f"GPT4 Correct User: {single_turn_prompt}<|end_of_turn|>GPT4 Correct Assistant: {res}<|end_of_turn|>GPT4 Correct User: {follow_up_question}<|end_of_turn|>GPT4 Correct Assistant:"
    # res=generate_response_starling_LM_7B_beta(multi_turn_prompt, model, tokenizer)
    messages = [
      {"role": "system", "content": "You are a pirate chatbot who always responds in pirate speak!"},
      {"role": "user", "content": map_prompt},
    ]
    res=getanswer(messages,pipeline_)
    return res
...
# def locate_error_(source_code,target_code):
    ...
    res=getanswer(messages,pipeline_)
    # single_turn_prompt = f"GPT4 Correct User: {prompt1}<|end_of_turn|>GPT4 Correct Assistant:"
    # res=generate_response_starling_LM_7B_beta(single_turn_prompt, model, tokenizer)
    # response, messages, extra_response_count=get_api_continue_response(messages)
    javascript_reference_document=get_knowledge_base(res)
    ...
    prompt2=prompt2.replace("{similar APIs and operators}",res)
    # prompt2=prompt2.replace("{javascript reference document}",javascript_reference_document)
    messages = [
        {"role": "system", "content": "You are a pirate chatbot who always responds in pirate speak!"},
        {"role": "user", "content": prompt2},
        ]
    res2=getanswer(messages,pipeline_)
    # single_turn_prompt = f"GPT4 Correct User: {prompt2}<|end_of_turn|>GPT4 Correct Assistant:"
    # res2=generate_response_starling_LM_7B_beta(single_turn_prompt, model, tokenizer)
    # messages.append({"role": "assistant", "content": assistant_reply})
    # ,
    messages=[]
    prompt3=get_prompt("located_error_prompt3.txt")
    prompt3 = prompt3.replace("{api and operator comparison}",res2)
    prompt3=prompt3.replace("{javascript reference document}",javascript_reference_document)
    messages = [
        {"role": "system", "content": "You are a pirate chatbot who always responds in pirate speak!"},
        {"role": "user", "content": prompt3},
        ]
    res3=getanswer(messages,pipeline_)
    # single_turn_prompt = f"GPT4 Correct User: {prompt3}<|end_of_turn|>GPT4 Correct Assistant:"
    # res3=generate_response_starling_LM_7B_beta(single_turn_prompt, model, tokenizer)
    ...
    # single_turn_prompt = f"GPT4 Correct User: {prompt4}<|end_of_turn|>GPT4 Correct Assistant:"
    messages = [
        {"role": "system", "content": "You are a pirate chatbot who always responds in pirate speak!"},
        {"role": "user", "content": prompt4},
        ]
    res4=getanswer(messages,pipeline_)
    # res4=generate_response_starling_LM_7B_beta(single_turn_prompt, model, tokenizer)

    # ，，，
    # messages=[]
    prompt5=get_prompt("located_error_prompt5.txt")
    prompt5 = prompt5.replace("{reference document flag}",res3)
    messages.append({"role": "user", "content": prompt5})
    # multi_turn_prompt = f"GPT4 Correct User: {prompt4}<|end_of_turn|>GPT4 Correct Assistant: {res4}<|end_of_turn|>GPT4 Correct User: {prompt5}<|end_of_turn|>GPT4 Correct Assistant:"

    # res5=generate_response_starling_LM_7B_beta(multi_turn_prompt, model, tokenizer)
    messages = [
        {"role": "system", "content": "You are a pirate chatbot who always responds in pirate speak!"},
        {"role": "user", "content": prompt4},
    ...
    res5=getanswer(messages,pipeline_)
    ...
    prompt6=prompt6.replace("{reference document flag}",res5)
    # single_turn_prompt = f"GPT4 Correct User: {prompt6}<|end_of_turn|>GPT4 Correct Assistant:"
    # res=generate_response_starling_LM_7B_beta(single_turn_prompt, model, tokenizer)
    messages = [
        {"role": "system", "content": "You are a pirate chatbot who always responds in pirate speak!"},
...
# def locate_error(source_code,target_code):
def locate_error_llama3(source_code,target_code):
    messages=[]
    input_token=0
    output_token=0
    ...
    messages.append({"role": "user", "content": prompt5})
    multi_turn_prompt = f"GPT4 Correct User: {prompt4}<|end_of_turn|>GPT4 Correct Assistant: {res4}<|end_of_turn|>GPT4 Correct User: {prompt5}<|end_of_turn|>GPT4 Correct Assistant:"
    messages=[{"role":"user","content":prompt4},{"role":"assistant","content":res4},{"role":"user","content":prompt5}]
    res5=chat_llama3_continue(messages)
